def registry = 'https://trialinu4x6.jfrog.io'
...
pipeline {                                    // 1  // Defines the start of the Jenkins pipeline block
    ...
    environment {                             // 2  // Defines environment variables for the pipeline
        PATH = "/var/lib/maven/bin:$PATH"         // Adds Maven's path to the system's PATH variable
                ...
                script {                      // 16  // Allows running custom Groovy script inside the pipeline
                    ...
                    def uploadSpec = """{
                          "files": [
                            {
                              "pattern": "jarstaging/(*)",
                              "target": "devopsvd-libs-release-local/{1}",
                              "flat": "false",
                              "props": "${properties}",
                              "exclusions": [ "*.sha1", "*.md5"]
                            }
